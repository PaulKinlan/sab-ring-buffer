class RingBuffer {
  ...
  */
  ...
  append(data) {
    Atomics.wait(this._header, RingBuffer.HEADER.WRITE, 0);
    {
      // Transaction scoped for my reading.
      Atomics.set(this._header, RingBuffer.HEADER.WRITING, 1);
    ...
    Atomics.set(this._header, RingBuffer.HEADER.WRITING, 0);
  }

  // Reads the next byte of data
  read() {
    Atomics.wait(this._header, RingBuffer.HEADER.READING, 0);
    {
      // Transaction scoped for my reading.
      Atomics.set(this._header, RingBuffer.HEADER.WRITING, 1);
  ...
  * readToHead() { // Feels odd to have to create a buffer the same size as the buffer. Just iterate.
    let data;
